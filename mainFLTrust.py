...
from Server.ServerBulyan import Bulyan
...
def main(dataset, algorithm,batch_size, learning_rate, lamda, num_glob_iters,
         local_epochs, optimizer, numusers, K, times, gpu,n_attackers,attacker_type,lambda_JR,num_GoodUsers,iid,resume):
    ...
    for i in range(times):
        ...
        if(algorithm == "FLTrust"):
            server = FLTrust(device, dataset, algorithm, model, batch_size, learning_rate,num_glob_iters, local_epochs, optimizer, numusers, i,n_attackers,attacker_type,lambda_JR,num_GoodUsers,iid,resume)
        server.train()
        server.test()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_global_iters", type=int, default=150)
