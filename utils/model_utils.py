...
def read_cifar10_data(iid=True):
    transform = transforms.Compose(
...
def read_svhn_data(iid=True):
    transform = transforms.Compose(
        [transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    data_loc = config.svhn_dir
    trainset = torchvision.datasets.SVHN(
        root=data_loc, split='train', download=True, transform=transform, target_transform=target_transform)
    testset = torchvision.datasets.SVHN(
        root=data_loc, split='test', download=True, transform=transform, target_transform=target_transform)
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=len(trainset.data), shuffle=False)
    testloader = torch.utils.data.DataLoader(
        testset, batch_size=len(testset.data), shuffle=False)
...
def read_data(dataset, iid):
    ...
    if (dataset == "FashionMNIST"):
        ...
        return clients, groups, train_data, test_data
    if (dataset == "Cifar10"):
        clients, groups, train_data, test_data = read_cifar10_data(iid)
        return clients, groups, train_data, test_data
    if (dataset == "Cifar100"):
        clients, train_data = read_cifar100_data_iid()
