...
from Server.ServerScaffold import Scaffold
...
def main(dataset, algorithm,batch_size, learning_rate, lamda, num_glob_iters,
         local_epochs, optimizer, numusers, K, times, gpu,n_attackers,attacker_type,lambda_JR,num_GoodUsers,iid,resume,resumeround):
    ...
    for i in range(times):
        ...
        # select algorithm
        modelpath="/home/Wudi-007/ModelJacobian/rebuttal/"
        model_path = os.path.join("rebuttal")
        a=""
        dict_attacker_type=attacker_type
        if n_attackers==0:
            dict_attacker_type="Noattack"
        if num_GoodUsers>0:
            a="_"+"WithModelJacobian"
        modelpath=modelpath+dict_attacker_type+"_"+algorithm + a+"_"+str(resumeround-1)+".pt"
        if resume== True:
            model=torch.load(modelpath)
            print("Resume from previous round")
        else:
            modelpath=modelpath+dict_attacker_type+"_"+algorithm + a+"_"+str(resumeround)+".pt"
            torch.save(model,modelpath)
            #model=torch.load(model,  os.path.join(model_path,dict_attacker_type+"_"+algorithm + a+".pt"))
            model=torch.load(modelpath)
            print("Start a new round of training")
        #torch.save(model,  "rebuttal/Cifar10_Scaffold.pt")
        if(algorithm == "Scaffold"):
            server = Scaffold(device, dataset, algorithm, model, batch_size, learning_rate,num_glob_iters, local_epochs, optimizer, numusers, i,n_attackers,attacker_type,lambda_JR,num_GoodUsers,iid,resumeround)
        server.train()
        server.test()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--algorithm", type=str, default="Scaffold",choices=["pFedMe", "PerAvg", "FedAvg","Scaffold"]) 
    ...
    parser.add_argument("--resumeround", type=int, default=0, help="resumeround")
    ...
    print("ResumeRound       : {}".format(args.resumeround))
    print("=" * 80)
