...
from Server.ServerMedian import Median
from Server.ServerKrum import Krum
...
def main(dataset, algorithm,batch_size, learning_rate, lamda, num_glob_iters,
         local_epochs, optimizer, numusers, K, times, gpu,n_attackers,attacker_type,lambda_JR,num_GoodUsers,iid,multi_k):
    ...
    for i in range(times):
        ...
        # select algorithm
        if(algorithm == "Krum"):
            server = Krum(device, dataset, algorithm, model, batch_size, learning_rate,num_glob_iters, local_epochs, optimizer, numusers, i,n_attackers,attacker_type,lambda_JR,num_GoodUsers,iid,multi_k)
        server.train()
        server.test()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="synthetic", choices=["MNIST", "synthetic", "Cifar10","Cifar100","SVHN","FashionMNIST"])
    ...
    parser.add_argument("--algorithm", type=str, default="Krum",) 
    ...
    parser.add_argument("--multi_k", type=bool, default=False, help="multi_k")
    ...
    print("MultiK       : {}".format(args.multi_k))
